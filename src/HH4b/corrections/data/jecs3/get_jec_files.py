...
def main():
    ...
    CORRECTIONS = {
        ...
        "MC": {"2023mc": "Summer23Prompt23_V3_MC", "2023BPixmc": "Summer23BPixPrompt23_V3_MC"},
    }
    ...
    FILE_TEMPLATES = {
        "AK8": {
            ...
            "data": ["L2Relative_AK8PFPuppi.txt", "L2L3Residual_AK8PFPuppi.txt"],
        },
        "AK4": {
            "MC": [
            ...
            "data": ["L2Relative_AK4PFPuppi.txt", "L2L3Residual_AK4PFPuppi.txt"],
        },
    }
    ...
    # Generate all URLs
    gitpaths = {
        "data": {dataset: build_urls("data", dataset) for dataset in CORRECTIONS["data"]},
        "MC": {dataset: build_urls("MC", dataset) for dataset in CORRECTIONS["MC"]},
    }
    ...
    def build_jec_inputs():
        ...
        for key, value in CORRECTIONS["data"].items():
            data_out["AK4"][key] = [
                file for file in DATA_files if value[:-6] in file and "AK4" in file
            ]  # :-6 on value removes up to and including the # in V#_DATA
            data_out["AK8"][key] = [
                file for file in DATA_files if value[:-6] in file and "AK8" in file
            ]

        for key, value in CORRECTIONS["MC"].items():
            mc_out["AK4"][key] = [
                file for file in MC_files if value[:-4] in file and "AK4" in file
            ]  # :-4 on value removes up to and including the # in V#_MC
            mc_out["AK8"][key] = [file for file in MC_files if value[:-4] in file and "AK8" in file]

        mc_out["AK4"]["2023mc"] += [
            file
            for file in MC_files
            if "Summer23Prompt23RunCv1234_JRV1_MC" in file and "AK4" in file
        ]
        mc_out["AK8"]["2023mc"] += [
            file
            for file in MC_files
            if "Summer23Prompt23RunCv1234_JRV1_MC" in file and "AK8" in file
        ]

        mc_out["AK4"]["2023BPixmc"] += [
            file
            for file in MC_files
            if "Summer23BPixPrompt23_RunD_JRV1_MC" in file and "AK4" in file
        ]
        mc_out["AK8"]["2023BPixmc"] += [
            file
            for file in MC_files
            if "Summer23BPixPrompt23_RunD_JRV1_MC" in file and "AK8" in file
        ]
